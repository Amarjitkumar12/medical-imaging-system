...
// Generate PDF route (frontend compatibility)
app.post('/generate-pdf', authenticateToken, upload.array('images'), async (req, res) => {
    let browser;
    try {
        const { patientData, imagesPerPage, clinicData, reportType, imageSize, previewOnly } = req.body;
        const images = req.files || [];

        console.log(`📋 Generating ${reportType} report for patient: ${patientData.name} (Preview: ${previewOnly})`);
        ...
        if (previewOnly) {
            // For preview, just return the PDF
            res.setHeader('Content-Type', 'application/pdf');
            res.send(pdf);
        } else {
            // For download, save to database and return with filename
...
app.post('/generate-from-saved/:id', authenticateToken, async (req, res) => {
    let browser;
    try {
        const reportId = req.params.id;
        
        // Find the saved report in database (only from this clinic)
        const report = await Report.findOne({ 
            _id: reportId, 
            clinicId: req.user._id 
        }).populate('patientId', 'name uhid age sex referredBy');
        ...
        // Get images from database (only from this clinic)
...
async function startServer() {
    try {
        ...
        app.listen(port, () => {
            ...
            console.log(`🔐 Multi-clinic authentication enabled`);
